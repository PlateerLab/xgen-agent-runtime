...
def test_첫_턴_표면은_스물몇_개를_넘지_않는다():
    ...
    #
    # 28 (2026-09-17, 27 →): ToolBatch 하나 — 같은 도구를 입력 목록으로 한 왕복에
    # 실행한다. 목록 작업(상품 N개 검색 등)을 항목마다 따로 부르면 왕복이 N배가 되고,
    # 실측(dev 위배상품 점검 8회)에서 검색 도구 호출 88회·턴당 왕복 약 19회가 그
    # 습관에서 나왔다. 첫 턴에 없으면 그 습관이 처음부터 생긴다. 스키마 하나로 왕복
    # 수를 줄이는 교환이라 표면을 넓히는 쪽보다 줄이는 쪽이다.
    #
    # 29 (2026-09-17, 28 →): TableExport 하나 — 호스트(xgen-workflow)가 등록하는 표
    # 내보내기. "엑셀로 정리" 마무리마다 pip install → openpyxl 스크립트 → ls 확인 →
    # 파일 전달 경로 탐색으로 왕복 2~4회를 쓰던 것을 한 호출로 줄인다. 이 도구를
    # 등록하지 않는 호스트에서는 이름만 있고 표면에 오르지 않는다.
    #
    # 이 선을 다시 올리려면 **한 도구씩** 이유를 여기 적어야 한다. 이 주석이
    # 길어지는 것이 곧 표면이 넓어졌다는 신호다.
    assert len(TURN_ONE_TOOLS) <= 29, sorted(TURN_ONE_TOOLS)
