...
_MAP = (
    ...
    "- ForgeTool / ListForgedTools / DeleteForgedTool — turn a script in your workspace "
    "into a reusable tool (stdin JSON → stdout JSON; verified once with test_input).\n"
...
class SelfExtendGuideTool(Tool):
    ...
    def description(self) -> str:
        ...
        return (
            "START HERE to extend yourself: install packages that persist (PythonEnv / "
            "SystemPackages), turn a script into a reusable tool (ForgeTool), or edit your "
            "own workflow graph to gain capabilities like document search (WorkflowSelf). "
            "Calling this OPENS those tools and returns a short map. Free, instant."
        )
